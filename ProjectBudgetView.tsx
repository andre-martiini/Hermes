        ...
        <div className="p-6 space-y-10">
           ...
           <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h3 className="text-xl font-bold text-slate-800">Teto Orçamental Inicial</h3>
                        <p className="text-slate-400 text-xs font-medium mt-1">Defina os valores aprovados para cada rubrica macro.</p>
                    </div>
                    {!isEditingBudget ? (
                        <button
                            onClick={() => setIsEditingBudget(true)}
                            className="text-indigo-600 hover:text-indigo-800 text-xs font-bold uppercase tracking-wider flex items-center gap-2"
                        >
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all"
                            >
                                Salvar Alterações
                            </button>
                        </div>
                            ...
                            {isEditingBudget ? (
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white border border-slate-200 rounded-xl py-2 pl-10 pr-4 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500"
                                    />
                                </div>
                            ) : (
           ...
           <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                <div className="lg:col-span-1 bg-indigo-600 rounded-2xl p-8 text-white shadow-xl shadow-indigo-200 relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-16 -mt-16"></div>
                    <div className="relative z-10">
                        <p className="text-[10px] font-bold uppercase tracking-wider opacity-70">Valor Total do Projeto</p>
                ...
                <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-4 hermes-stagger">
                    {(['custeio', 'capital', 'bolsas'] as const).map(rubrica => {
                        ...
                        return (
                            <div key={rubrica} className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col justify-between hermes-lift">
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{rubrica}</span>
                                        {isLow && balance > 0 && (
                                            <span className="px-2 py-1 bg-red-100 text-red-600 rounded-full text-[9px] font-bold uppercase tracking-wider animate-pulse">
                                                Atenção
                                            </span>
                                        )}
                                    </div>
                                    <p className={`text-2xl font-bold ${isLow ? 'text-red-500' : 'text-slate-800'}`}>
           ...
           <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h3 className="text-xl font-bold text-slate-800">Itens Planejados</h3>
                    ...
                    <button
                        ...
                        className="bg-slate-900 text-white px-5 py-3 rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-slate-800 transition-all flex items-center gap-2"
                    >
                    ...
                    <form onSubmit={handleAddItem} className="mb-8 bg-slate-50 p-6 rounded-2xl border border-slate-200 animate-in fade-in slide-in-from-top-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                            <div className="lg:col-span-2">
                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">Descrição do Item</label>
                                <input
                            ...
                            <div>
                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">Rubrica</label>
                                <select
                            ...
                            <div>
                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">Valor Unit. Est.</label>
                                <input
                                    type="number"
                        ...
                        <div className="flex justify-end gap-3">
                            <button
                                ...
                                className="px-4 py-2 text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-slate-600"
                            >
                                Cancelar
                            </button>
                            <button
                                type="submit"
                                className="bg-indigo-600 text-white px-6 py-2 rounded-xl text-xs font-bold uppercase tracking-wider shadow-lg shadow-indigo-200 hover:bg-indigo-700"
                            >
                                Adicionar Item
                            </button>
                        </div>
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-100">
                    <table className="w-full">
                        <thead className="bg-slate-50 border-b border-slate-100">
                            <tr>
                                <th className="p-4 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider">Item</th>
                                <th className="p-4 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider">Rubrica</th>
                                <th className="p-4 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider">Est. Total</th>
                                <th className="p-4 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider">Gasto Real</th>
                                <th className="p-4 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider">Economia</th>
                                <th className="p-4 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider">Status</th>
                            </tr>
                        </thead>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-xl font-bold text-slate-800">Ajuste de Rubricas</h3>
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total Disponível:</span>
                            <span className="text-sm font-bold text-indigo-600">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalProjectValue)}</span>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleReallocate} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-4">
                                <div>
                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">Novo Custeio</label>
                                ...
                                <div>
                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">Novo Capital</label>
                                    <div className="relative mt-1">
                                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm">R$</span>
                                        <input
                                            type="number"
                                ...
                                <div>
                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">Bolsas (Ajuste se necessário)</label>
                                    <div className="relative mt-1">
                                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm">R$</span>
                                        <input
                                            type="number"
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">Justificativa do Ajuste</label>
                                    <textarea
                                ...
                                <div className={`p-4 rounded-xl border ${Math.abs((simplifiedBudgetForm.custeio + simplifiedBudgetForm.capital + simplifiedBudgetForm.bolsas) - totalProjectValue) < 0.01 ? 'bg-emerald-50 border-emerald-100' : 'bg-amber-50 border-amber-100'}`}>
                                    <div className="flex justify-between items-center">
                                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Nova Soma:</span>
                                        <span className={`text-sm font-bold ${Math.abs((simplifiedBudgetForm.custeio + simplifiedBudgetForm.capital + simplifiedBudgetForm.bolsas) - totalProjectValue) < 0.01 ? 'text-emerald-600' : 'text-amber-600'}`}>
                        ...
                        <button
                            type="submit"
                            className="w-full bg-slate-900 text-white py-4 rounded-2xl text-xs font-bold uppercase tracking-wider shadow-lg hover:bg-slate-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                ...
                {/* Histórico */}
                <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm h-full max-h-[500px] overflow-y-auto">
                    <h3 className="text-xl font-bold text-slate-800 mb-6">Histórico de Movimentações</h3>
                    <div className="space-y-4">
           ...
           <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
                <h3 className="text-xl font-bold text-slate-800 mb-6">Simulador de Sustentabilidade (Bolsas)</h3>
                <BurnRateSimulator
