...
export const ProjectsView = ({
    ...
    return (
      <div className="px-4 md:px-0">
        <button
          ...
          className="mb-6 flex items-center gap-2 text-slate-400 hover:text-indigo-600 font-bold text-[10px] uppercase tracking-wider transition-all group"
        >
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-card min-h-[700px] animate-in">
             {/* Header do Projeto - Estilo Premium */}
             <div className="p-8 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                      <span className="px-3 py-1 bg-indigo-100 text-indigo-600 rounded-full text-[9px] font-bold uppercase tracking-wider">Projeto Ativo</span>
                    </div>
                ...
                <div className="flex gap-4 items-center">
                  <div className="hidden md:block text-right">
                    <p className="text-[9px] font-bold text-slate-300 uppercase tracking-wider">Data de Início</p>
             ...
             {/* Abas de Navegação */}
             <div className="px-8 border-b border-slate-100 flex gap-6 overflow-x-auto">
                <button
                    onClick={() => setActiveTab('bolsistas')}
                    className={`py-4 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors whitespace-nowrap ${activeTab === 'bolsistas' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                >
                    Gestão de Bolsistas
                </button>
                <button
                    onClick={() => setActiveTab('orcamento')}
                    className={`py-4 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors whitespace-nowrap ${activeTab === 'orcamento' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                >
                    Planejamento Orçamental
                </button>
                <button
                    onClick={() => setActiveTab('aquisicoes')}
                    className={`py-4 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors whitespace-nowrap ${activeTab === 'aquisicoes' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                >
  ...
  return (
    <div className="space-y-8 pb-20 px-4 md:px-0">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 hermes-stagger">
        {/* Lista de Projetos */}
        {projects.map(project => (
          <div
            key={project.id}
            onClick={() => setSelectedProjectId(project.id)}
            className="group bg-white border border-slate-200 rounded-2xl p-7 hermes-lift hover:border-indigo-400/50 cursor-pointer relative overflow-hidden"
          >
            ...
            <div className="relative z-10 space-y-5">
                ...
                <div>
                    <h4 className="text-xl font-bold text-slate-800 group-hover:text-indigo-700 transition-colors line-clamp-1">{project.nome}</h4>
                ...
                <div className="pt-5 border-t border-slate-100 flex items-center justify-between">
                    <div className="flex flex-col">
                      <span className="text-[9px] font-bold text-slate-300 uppercase tracking-wider">Criado em</span>
      ...
      {isCreating && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-md rounded-2xl shadow-card p-8 animate-in zoom-in-95">
                <h3 className="text-xl font-bold text-slate-900 mb-6">Criar Novo Projeto</h3>
