...
call gcloud config set project %PROJETO%
call gcloud services enable run.googleapis.com cloudbuild.googleapis.com artifactregistry.googleapis.com aiplatform.googleapis.com

echo.
echo [2/4] Repositorio de imagens (ignora "ja existe")...
call gcloud artifacts repositories create hermes --repository-format=docker --location=%REGIAO% 2>nul

echo.
echo [3/4] Build da imagem (Cloud Build, a partir da raiz)...
call gcloud builds submit --config services/video-worker/cloudbuild.yaml --substitutions=_IMAGEM=%IMAGEM% .
if %ERRORLEVEL% NEQ 0 (
...
call gcloud run jobs deploy hermes-video-worker --image=%IMAGEM% --region=%REGIAO% --service-account=%CONTA% --task-timeout=3600 --max-retries=0 --memory=2Gi --cpu=2 --tasks=1
