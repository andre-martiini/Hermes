# Registro de execução (arquivo) — plano-hermes-autonomo-2026-09-06

Arquivo dos blocos P00 a P02 sub-entrega 10/N-11/N (catch-up), movidos para
cá em 2026-09-09 (P02 sub-entrega 15/N) por um limite técnico real: o
arquivo ativo (docs/autonomia/execucao.md) tinha crescido a ponto de um
único bloco novo de conteúdo, somado ao histórico acumulado, exceder o
teto de tokens de saída de uma única chamada de escrita via Argos MCP (a
ferramenta de escrita não aceita diff/patch, só substituição integral do
arquivo -- ver docs/autonomia/execucao.md ou a skill argos-ship-feature
para o motivo) -- confirmado por duas tentativas reais que truncaram
silenciosamente o conteúdo enviado, e uma terceira que falhou com erro
explícito de limite de tokens de saída (64000). Nenhum conteúdo foi
perdido nem reescrito: este arquivo é uma cópia byte-a-byte da faixa
correspondente do arquivo ativo antes do corte. O arquivo ativo continua
sendo a fonte de verdade para tudo daqui pra frente e para as pendências
ainda abertas (que já eram restatadas em cada bloco novo, não vivem só
aqui). Ordem cronológica preservada; nada aqui é 'validado' que precise
ser reescrito.
