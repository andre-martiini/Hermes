# Registro de execução (arquivo) — plano-hermes-autonomo-2026-09-06 — P02 sub-entrega 12/N a 16/N

Arquivo dos blocos P02 sub-entrega 12/N a 16/N, movidos para cá em
2026-09-09 (durante a correção da sub-entrega P01 5/N). Motivo: uma
tentativa de corrigir 2 palavras no arquivo ativo
(docs/autonomia/execucao.md, então com 85701 bytes) derrubou
silenciosamente um bloco inteiro (P02 sub-entrega 17/N, ~16KB) durante a
retranscrição manual do conteúdo completo para a chamada de escrita via
Argos MCP, sem erro explícito da API -- diagnosticado só depois, por diff
byte a byte entre o commit remoto e o conteúdo local já verificado por
hash antes do envio. Ao recuperar, uma tentativa de reenviar tudo somado
num único arquivo de arquivo (159725 bytes, este conteúdo mais o arquivo
de arquivo já existente, docs/autonomia/execucao-archive-p00-a-p02sub11.md)
também falhou -- desta vez com erro explícito de limite de saída (64000
tokens de uma chamada). Por isso este é um arquivo de arquivo NOVO e
separado, em vez de mais conteúdo anexado ao arquivo de arquivo original:
cada escrita via Argos precisa reenviar o arquivo inteiro (sem
diff/patch), então um arquivo de arquivo que só cresce eventualmente fica
grande demais para qualquer escrita futura o tocar, mesmo só para
adicionar uma nota. Nenhum conteúdo foi perdido -- os blocos abaixo são
cópia byte-a-byte do arquivo ativo de antes deste corte, extraída por
script (não retranscrita à mão). O arquivo ativo
(docs/autonomia/execucao.md) continua sendo a fonte de verdade a partir
da sub-entrega 17/N; docs/autonomia/execucao-archive-p00-a-p02sub11.md
guarda P00 a P02 sub-entrega 11/N, sem nenhuma alteração nesta operação.
