...
O Gaspar já lê e envia mensagens de WhatsApp em nome do André, mas hoje toda mensagem enviada passa por confirmação manual dele antes de sair (fluxo `schedule_whatsapp_message` → `confirmar_acao`).

A ideia surgiu de uma necessidade concreta: quando o André está em reunião (ou indisponível por qualquer motivo), ele gostaria que o Gaspar atendesse quem escreve no WhatsApp como um secretário atenderia — não apenas com uma mensagem padrão de "estou ocupado", mas entendendo o assunto, fazendo perguntas de esclarecimento e só o interrompendo quando necessário.
...
Permitir que o Gaspar converse com terceiros no WhatsApp em nome do André durante períodos de indisponibilidade, coletando informação e resolvendo o que for de baixo risco sozinho, sem nunca comprometê-lo com uma decisão, uma confirmação ou uma promessa que só ele pode dar.
...
Quando alguém pede para marcar algo em um dia ou horário específico, o Gaspar consulta a agenda real do André:

- **Se ele estiver ocupado** naquele horário: o Gaspar pode informar isso diretamente (ex.: "nesse dia ele estará em [compromisso] das Xh às Yh"). É informação factual, sem risco de comprometê-lo.
- **Se ele estiver livre**: o Gaspar **não confirma disponibilidade nem fecha o compromisso**. Apenas informa que vai repassar o recado, e o André decide e confirma pessoalmente depois.
...
Quando o André sabe de antemão que está esperando resposta de alguém específico sobre um assunto específico, ele avisa o Gaspar com antecedência: quem é a pessoa e o que precisa saber da resposta dela.

Se esse contato escrever durante o período de indisponibilidade, o Gaspar não trata a mensagem como recado genérico: conduz a conversa buscando ativamente as informações combinadas, e ao final apresenta ao André um resumo do que a pessoa trouxe.
...
Essa fronteira segue o mesmo princípio da "autonomia graduada" já adotado no Gaspar: começar restrito ao que é seguro, e promover para autônomo conforme a métrica de acerto justificar.

## 5. O que exige mudança de código (não é só uma skill)

Uma skill do Claude sozinha não é suficiente, porque não dá presença contínua no WhatsApp nem guarda estado entre mensagens de sessões diferentes. É necessário trabalho no backend do próprio Gaspar (Cloud Functions / webhook):
...
A camada de raciocínio — como o Gaspar já delega ~90% das decisões ao Claude via MCP — pode ser justamente as regras da seção 3 e a tabela da seção 4, entregues como instrução ao Claude no momento em que o Gaspar o aciona para decidir como responder. Ou seja: a skill é o "cérebro" da decisão; o código é o que faz essa decisão entrar e sair do WhatsApp na hora certa.
...
- O que fazer quando a pessoa insiste em obter uma decisão imediata e o Gaspar não pode dar?
...
- **Precedente de LLM acionado pelo backend sem sessão interativa**: `functions/ai_notification_planner.py` já invoca um LLM fora de uma sessão MCP (o planejador proativo das 6h30). Isso resolve a maior dúvida técnica — como o Gaspar decide o que responder a uma mensagem recebida sem um humano com sessão aberta — sem precisar inventar infraestrutura nova do zero.
...
3. **Insistência da pessoa:** depois de 2 trocas sem resolver (número sugerido, ajustável), o Gaspar para de investigar e informa que vai repassar o recado — nunca entra em loop tentando "resolver" sozinho.
