...
| `sugerir_respostas` | Scheduler (`*/5 7-22 * * *`, America/Sao_Paulo, 256 MB) | Cria o rascunho de resposta (cartão de aprovação no Telegram) para conversas individuais de WhatsApp cujo prazo `due_at` em `respostas_sugeridas` venceu: 1 chamada do Gemini por chat, com limites por rodada e por dia. Atrás de `system/settings.atencao.resposta_sugerida.enabled` (padrão desligado). Ver `integracoes/whatsapp.md` |
