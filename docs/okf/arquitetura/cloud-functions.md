...
| `consolidar_personalidade` (`personal_diary.py`) | Scheduler (domingo 22h BRT) | Destila os diários da semana (+ ajustes pedidos nos diários + correções diretas em `ai_profile.personalidade_ajustes`, o sinal mais forte) num perfil de personalidade em `usuarios/{uid}.ai_profile.personalidade`, versionado. Logo depois de gravar, manda no Telegram o espelho semanal "🪞 O que percebi em você esta semana" (resumo + gatilhos/fontes de energia que entraram ou saíram) com "✏️ Corrigir" (`perfil:fix` — a próxima mensagem livre vira correção) e "👍 Está certo" (`perfil:ok`), tratados em `telegram_callbacks_perfil.py`. Mesma flag `personal_diary.enabled`. Removida em 26/09/2026 (PR #356) e restaurada no mesmo dia |
...
|---|---|---|
| `on_whatsapp_message_atencao` | Firestore on-create em `whatsapp_messages/{message_id}` | Roda os dois detectores a cada mensagem capturada, cada um isolado em seu proprio try/except. **`promessa_sem_retorno`**: mensagem `from_me` de texto que casa com um padrao de compromisso ("deixa comigo", "te aviso ainda hoje", ...) cria/renova um documento em `promessas_abertas`; uma mensagem `from_me` seguinte longa ou com midia fecha a promessa como `cumprida`. Controlada por `system/settings.atencao.promessa_sem_retorno.enabled`/`.horas`. **`audio_relevante`**: audio (`ptt`/`audio`) recebido em chat vinculado a uma acao ativa (via `tarefas.whatsapp_vinculos` ou, em chat individual, via `perfil_pessoas.whatsapp_chat_id` + `interacoes_pessoas`) vira item da fila `atencao` e enfileira/atualiza pedido autônomo em `agent_requests` (`consolidar_audio:{item_atencao_id}`); sequencias de audios em menos de 10 min no mesmo chat sao mescladas num item so e atualizam o payload do pedido pendente. Controlada por `system/settings.atencao.audio_relevante.enabled`/`.segundos_min`. Um terceiro passo, **`resposta_esperada` (modo sombra)**, grava em `sinais_resposta_sombra` as mensagens recebidas de quem uma etapa `aguardando_terceiro` aguarda (só ids, nunca o texto), lendo o índice `system/esperas_index`; controlado por `system/settings.atencao.resposta_esperada.enabled` |
