...
| `mcp_audit_log` | Registro de auditoria de cada chamada de ferramenta MCP (`uid`, `tool`, `arguments`, `latency_ms`, `is_error`: bool, `timestamp`). Usado para telemetria de uso e cálculo de taxa de erro/latência pela retrospectiva semanal. | `functions/mcp_server.py` (`_audit_log`); consultado por `functions/retro_agente.py` |
...
| `whatsapp_outbox` | Fila de mensagens WhatsApp agendadas e rascunhos com aprovação via Telegram ou WhatsApp próprio (`to_number`, `content`, `status`: `pending`\|`sending`\|`sent`\|`failed`\|`notified`\|`canceled`\|`aguardando_aprovacao`\|`aguardando_janela`\|`descartado`\|`expirado`, `motivo`, `acao_id`, `item_atencao_id`, `origem`, `tipo`: string default `"outro"`, `foi_editado`: bool default `false`, `envio_liberado_em`: timestamp opcional gravado para rascunhos de tipos promovidos, `destinatario_nome`, `telegram_message_id`, `aprovado_via`: `"telegram"`\|`"whatsapp"`\|`"janela_automatica"`, `scheduled_for`, `created_at`, `aprovado_em`, `descartado_em`, `expirado_em`, `notified_at`: gravado por `dispatch_scheduled_whatsapp_messages` ao mandar o card manual, `canceled_at`/`canceled_motivo`: gravados por `cancelar_envio` — MCP (`cancelar_envio_whatsapp`) ou callback Telegram `wa_cancel:{id}`). Coordenação de quem envia (worker local vs. card Telegram) via `system/settings.whatsapp_auto_send_enabled` + heartbeat | `schedule_whatsapp_message` e `criar_rascunho_whatsapp` (`tools`); `cancelar_envio_whatsapp`/`descartar_rascunho_whatsapp` (cancelamento pré-entrega, um par por fluxo); `liberar_rascunhos_promovidos` (sweep periódico); callbacks Telegram `outbox:{id}:{ok\|edit\|no}` e `wa_cancel:{id}`; resposta `from_me` no self-chat (`whatsapp_owner_chat_id`); consumida por `services/whatsapp-capture` (cron) ou `dispatch_scheduled_whatsapp_messages` |
