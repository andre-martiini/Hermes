...
|---|---|---|
| `email_action_suggestions` | Sugestões de vínculo entre e-mails recebidos e ações (`tarefas`) em andamento/stand-by, geradas por IA e confirmadas via Telegram. ID do doc = ID da mensagem do Gmail (dedupe estrutural). Campos: `google_message_id`, `subject`, `sender`, `snippet`, `internal_date`, `analyzed_at`, `model`, `related`, `confidence` (0-1), `task_id`/`task_titulo`/`task_status` (se `related`), `resumo`, `nota_sugerida`, `reativar_sugerido`, `status` (`no_match`\|`pending`\|`applied`\|`applied_reactivated`\|`dismissed`\|`expired`), `telegram_sent`, `sent_at`, `applied_at`, `decided_at`. Ver [proposta original](/docs/okf/propostas/vinculo-email-acao.md). | `email_action_linker.py` (`link_emails_to_actions`, chamada no fim de `run_full_sync`); status decidido pelo callback `emlink:{msgId}:{ok\|on\|no}` em `hermes_core_logic.py` |
