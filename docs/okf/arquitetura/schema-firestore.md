...
| `respostas_sugeridas` | Um documento por chat com resposta sugerida agendada ou já tratada (id = `chat_id`): `chat_id`, `chat_name`, `ultima_msg_id`, `estado` (`agendada`\|`rascunhada`\|`ignorada`), `due_at` (só enquanto agendada; a consulta é `due_at <= agora`), `motivo_ignorada` (`ja_respondida`, `antiga`, `secretario_ativo`, `ja_ha_rascunho`, `nao_responder`, `precisa_do_andre`, `reprovado_*`, `llm_sem_resposta_valida`, `falha_ao_criar_rascunho`, `excecao`), `motivo_llm`, `outbox_id`, `atualizado_em`, `processado_em`. | `resposta_sugerida.agendar` (trigger de mensagem) e `processar_vencidas` (`sugerir_respostas`) |
| `respostas_sugeridas_contadores` | Contadores por dia (id `AAAA-MM-DD`, fuso America/Sao_Paulo): `chamadas_llm` e `rascunhos`, para o limite diário. | `resposta_sugerida` |
