...
description: Coleções do Firestore usadas pelo Gaspar, agrupadas por domínio, com campos principais, quem escreve em cada uma e suas relações.
resource: types.ts
...
- **Campos-chave:** `titulo`, `projeto`, `area_tematica`, `data_inicio`, `data_limite`, `data_conclusao`, `status` (`em andamento`\|`stand-by`\|`concluído`), `execution_lane`, `acompanhamento[]`, `chat_history[]`, `plano_acao[]`, `artefatos_kg[]`, `kg_tags[]`, `concept_node_id`, `horario_inicio`/`horario_fim`, `reminders[]`, `origem` (`manual`\|`audio`\|`whatsapp`), `sync_status`, `knowledge_item_ids[]`, `email_link_optout` (opt-out do vínculo automático de e-mail por IA — ver `email_action_linker.py`), `whatsapp_vinculos[]` (contatos/grupos de WhatsApp vinculados manualmente à ação — matching determinístico usado por `whatsapp_ingest.py`, ver §3 de `docs/okf/integracoes/whatsapp.md`), `github_repos_vinculados[]` (repositórios GitHub no formato `"owner/repo"` vinculados manualmente à ação — matching determinístico usado por `githubWebhook` em `main.py`), `contexto_agente` (map com `resumo`, `pessoas_chave[]`, `onde_esta_o_codigo`, `ultimas_decisoes[]`, `travas[]`, `atualizado_em` mantido automaticamente por `on_tarefa_written_contexto_agente` em ações críticas), `last_processed_contexto_hash` (MD5 do texto fonte processado pelo trigger para deduplicação).
...
### `usuarios`
Perfis de IA por usuário: criado por `_bootstrap_user_ai_profile`, sinais de prompt atualizados por `_save_user_profile_signal` (`ai_profile.historico_deduzido`, rolling últimos 10). Campo `ai_profile.personalidade` — traços/estilo/rotinas destilados semanalmente do diário pessoal por `consolidar_personalidade` (`personal_diary.py`), com `personalidade_historico[]` (últimas 6 versões), `personalidade_atualizada_em` e `personalidade_ajustes[]` (correções diretas do usuário pelo botão "✏️ Corrigir" do espelho semanal: `texto`, `em`, `versao_perfil`; últimas 20; as até 10 mais recentes das últimas 8 semanas entram no prompt da consolidação seguinte como o sinal mais forte). Lido por `_format_ai_profile_for_prompt` (copiloto web/Godmode) e `context.py:_format_user_profile` (ponte de voz) — grava uma vez, propaga para as três superfícies. Também vai compacto (`perfil_pessoal`: resumo, estilo, gatilhos, fontes de energia, rotinas, `atualizado_em`) no `obter_estado_atual` do MCP, a partir da mesma leitura que `morning_summary._coletar_perfil` já faz.
...
| `atencao` | Fila unificada de atenção: itens e pendências que demandam decisão ou acompanhamento do dono (`origem`, `tipo`, `prioridade`, `titulo`, `resumo`, `acao_id`, `etapa_id`, `pessoa`, `prazo`, `evidencia`, `sugestao`, `estado`, `chave_dedupe`, `criado_em`, `atualizado_em`, `resolvido_em`, `desfecho`, `avaliado_interrupcao_em`). ID do doc = `chave_dedupe` determinística. Índice composto: `estado ASC, prioridade ASC, prazo ASC`. | `functions/atencao.py` (`detectar_atencao_acoes`, scheduler a cada 30 min; `avaliar_interrupcao_atencao`, sweep a cada 1 min em `check_and_send_reminders`); resolvida via MCP tool `resolver_item_atencao`; lida por `obter_fila_atencao` e `obter_estado_atual` |
...
| `sinais_resposta_sombra` | Modo sombra do sinal de resposta esperada: mensagem recebida de quem alguma etapa `aguardando_terceiro` aguarda (`message_doc_id`, `chat_id`, `is_group`, `message_type`, `message_ts`, `matches[]` com `acao_id`, `etapa_id`, `aguardando_de`, `origem` e `confianca` `alta`\|`media`\|`baixa`, `truncados`, `modo`). ID do doc = id da mensagem. Nunca guarda o texto: para rotular, ler `whatsapp_messages`. | `functions/resposta_esperada.py` (`processar_mensagem`, chamada por `on_whatsapp_message_atencao`) |
| `sinais_resposta_contadores` | Contadores por dia UTC do modo sombra (`avaliadas`, `com_match`, `conf_alta`, `conf_media`, `conf_baixa`, `suprimidas_grupo_sem_autor`), com `Increment`; a entrega do gatilho é at-least-once, então podem superestimar. ID do doc = `YYYY-MM-DD`. | `functions/resposta_esperada.py` |
| `system/esperas_index` | Índice das esperas (`entradas[]`: `acao_id`, `etapa_id`, `aguardando_de`, `nomes[]`, `chat_ids[]`, `grupos[]`, `desde`; mais `esperas_terceiros`, `esperas_dono_ou_ia`, `atualizado_em`). `desde` é o primeiro instante em que o índice viu a espera, pois o plano não guarda o carimbo de entrada. Só é regravado quando algo muda. Interruptor: `system/settings.atencao.resposta_esperada.enabled` (padrão desligado). | `resposta_esperada.reconstruir_indice`, chamada por `run_full_sync` (`scheduled_sync`, a cada 60 min) |
...
| `system_reports` | Textos do relatório diário de custos (`custos_{YYYY-MM-DD}`, dia do bloco GCP: `tipo`, `dia`, `resumo`, `detalhe`, `created_at`), lidos pelos botões "📋 Ver detalhes"/"↩️ Resumo" | `relatorio_diario_custos` (`cost_report.py`); leitura em `telegram_callbacks_custos.py` |
| `scheduled_notifications` | Fila de notificações agendadas pelo planejador proativo de IA e pelo sweep de interrupção de atenção (`title`, `message`, `category`, `send_at`, `status`: `pending`\|`sent`\|`failed`, `source`: `"ai_planner"`\|`"atencao_interrupcao"`, `motivo`, `atencao_id`, `feedback`: `null`\|`useful`\|`dismissed`) — enviadas ao Telegram com botões de feedback, não passam pela coleção `notificacoes` | `ai_notification_planner.py` (`propor_notificacao`, `ai_notification_planner_daily`), `atencao.py` (`avaliar_interrupcao_atencao`); consumida e marcada por `dispatch_pending_ai_notifications` (chamada de dentro de `check_and_send_reminders`); `feedback` atualizado pelo callback `ai_notif:{id}:{useful\|dismiss}` em `hermes_core_logic.py` |
| `reagendamentos_propostos` | Propostas de reagendamento em lote de ações atrasadas geradas pela revisão semanal (`items[]`, `justificativa`, `status`: `pending`\|`aplicado`\|`descartado`, `total_acoes`, `nova_data_inicio`, `criado_em`, `aplicado_em`, `descartado_em`, `resultado_aplicacao`). ID do doc gerado automaticamente. Trava de proposta única: no máximo 1 pendente por vez. A aplicação real só ocorre sob clique explícito no Telegram. | `functions/revisao_semanal.py` (`revisar_semana_propor_reagendamento`, scheduler segundas 05:15 BRT); status e aplicação decididos pelo callback `reagendamento_lote:{id}:{aplicar\|descartar}` em `hermes_core_logic.py` (executa `confirmarReagendamentoEmLote` em processo via `invoke_callable`) |
| `retros_agente` | Histórico das retrospectivas semanais de execuções do agente e ferramentas MCP (`periodo_inicio`, `periodo_fim`, `resumo`, `metricas`: dict com agregados brutos de rotinas e tools, `proposta_pop`, `criado_em`). ID do doc determinístico: `retro_YYYY_MM_DD`. | `functions/retro_agente.py` (`retro_semanal_agente`, scheduler domingos 20:00 BRT) |
| `promocoes_autonomia_sugeridas` | Sugestões de promoção de autonomia por tipo de rascunho de WhatsApp (tipos com taxa alta de envio sem edição) para transição para envio autônomo com janela de cancelamento (`tipo`, `status`: `pendente`\|`aceita`\|`adiada`\|`nunca`, `amostra`, `aprovados_sem_edicao`, `taxa_sem_edicao`, `sugerida_em`, `decidida_em`, `decisao`). ID do doc determinístico = `tipo`. | `functions/retro_agente.py` (`retro_semanal_agente`, hook determinístico); status e promoção decididos via MCP tool `decidir_promocao_autonomia` |
| `mcp_audit_log` | Registro de auditoria de cada chamada de ferramenta MCP (`uid`, `tool`, `arguments`, `latency_ms`, `is_error`: bool, `timestamp`, `result_chars`: tamanho da resposta em caracteres, só nas chamadas de execução direta e a partir de 20/09/2026). Usado para telemetria de uso e cálculo de taxa de erro/latência pela retrospectiva semanal. | `functions/mcp_server.py` (`_audit_log`); consultado por `functions/retro_agente.py` |
...
| `whatsapp_outbox` | Fila de mensagens WhatsApp agendadas e rascunhos com aprovação via Telegram ou WhatsApp próprio (`to_number`, `content`, `status`: `pending`\|`sending`\|`sent`\|`failed`\|`aguardando_aprovacao`\|`aguardando_janela`\|`descartado`\|`expirado`, `motivo`, `acao_id`, `item_atencao_id`, `origem`, `tipo`: string default `"outro"`, `foi_editado`: bool default `false`, `envio_liberado_em`: timestamp opcional gravado para rascunhos de tipos promovidos, `destinatario_nome`, `telegram_message_id`, `aprovado_via`: `"telegram"`\|`"whatsapp"`\|`"janela_automatica"`, `scheduled_for`, `created_at`, `aprovado_em`, `descartado_em`, `expirado_em`). Coordenação de quem envia (worker local vs. card Telegram) via `system/settings.whatsapp_auto_send_enabled` + heartbeat | `schedule_whatsapp_message` e `criar_rascunho_whatsapp` (`tools`); `liberar_rascunhos_promovidos` (sweep periódico); callbacks Telegram `outbox:{id}:{ok\|edit\|no}`; resposta `from_me` no self-chat (`whatsapp_owner_chat_id`); consumida por `services/whatsapp-capture` (cron) ou `dispatch_scheduled_whatsapp_messages` |
...
| `system/api_keys`, `system/settings`, `system/config`, `system/sync`, `system/google_credentials`, `system/file_search`, `system/cost_controls`, `system/mcp_access` | Documentos de configuração e estado global, na coleção `system`. `system/api_keys` inclui `telegram_bot_token`, `github_webhook_secret` (segredo HMAC-SHA256 para validação de webhooks em `githubWebhook`), etc. `system/settings` inclui: flags de detectores de atenção (`atencao.promessa_sem_retorno`, `atencao.audio_relevante`), `whatsapp_auto_send_enabled`, `whatsapp_owner_chat_id` (chat_id `@c.us` do próprio dono para aprovação/descarte/edição de rascunhos no WhatsApp), etc. `system/mcp_access` guarda controles de acesso e autonomia MCP: `tipos_promovidos[]` (tipos de mensagem do outbox promovidos para envio com janela de cancelamento) e `janela_cancelamento_min` (duração em minutos da janela antes da auto-liberação, padrão 10). | admin/setup scripts; `on_sync_request`; `start_file_indexing`; tool `decidir_promocao_autonomia` |
