...
| `whatsapp_digests` | Digests vetorizados de janelas de conversa do WhatsApp classificadas como `acao`/`conhecimento` (nunca mensagem a mensagem) — `chat_id`, `chat_name`, `resumo`, `topicos[]`, `relevancia`, `itens_de_acao[]`, `decisoes[]`, `datas_mencionadas[]`, `embedding` (Vector 768) | `whatsapp_ingest.py:triage_whatsapp_messages`; lida por `whatsapp_ingest.py:buscar_conversas_whatsapp` (tool do copiloto) |
