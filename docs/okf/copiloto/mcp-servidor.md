...
- **Salvaguardas:** recusa conteúdo vazio, acima de 2 MB, arquivo na lixeira e arquivo sem permissão de edição, antes de gravar. **Só atualiza arquivo do próprio dono** (`ownedByMe`; item de Drive compartilhado é recusado pelo `driveId`, porque a API não preenche `ownedByMe` ali), para uma injeção de prompt não reescrever documento de colega. **Recusa conteúdo com menos de 40% do tamanho atual** (a tool exige o documento COMPLETO, e um envio incompleto apagaria o resto em silêncio); `permitir_reducao=true` libera o encurtamento de propósito. Só vale a partir de 500 caracteres/bytes; no Doc compara o texto visível (HTML sem tags, `script` e `style`) com o texto exportado. O `conteudo` **não vai para o `mcp_audit_log`** (só `chars` e `sha256`, em `mcp_server._ARGUMENTOS_REDIGIDOS`): duplicaria o texto do documento no Firestore e, acima de 1 MiB, a auditoria da chamada falharia em silêncio. Depois de gravar, confere o `md5Checksum` (texto) ou que o Doc não ficou vazio (conversão que falha em silêncio); se não confere, devolve erro e manda restaurar em Arquivo > Histórico de versões.
- **Concorrência otimista (`esperado_modificado_em`, opcional):** quem lê o arquivo antes de reescrevê-lo chama o `get_file_metadata` do conector do Drive **antes** de ler o conteúdo e passa o `modifiedTime` em `esperado_modificado_em`. Se o arquivo mudou desde então, a tool recusa sem gravar e manda reler (ler o metadata antes do conteúdo é o lado seguro: uma edição no meio só gera recusa, nunca perda). Depois de uma escrita, o `modificado_em` devolvido serve para a próxima. Comparação **exata** do instante (formato RFC 3339 com fuso; sem tolerância). A checagem é um `files.get` logo antes do `update`; o Drive não tem escrita condicional para arquivo, então resta uma janela de milissegundos. Se não der para conferir (erro do Drive), recusa. **Armadilha medida em 20/09/2026:** num Google Doc o Drive re-carimba o `modifiedTime` 0,3–0,8 s depois do `update`, então o valor que o próprio `update` devolve fica defasado; por isso `modificado_em` é relido depois de gravar e verificar (arquivo de texto não tem a diferença). Sem o parâmetro não há checagem (opcional para não quebrar cliente com esquema em cache).
- **Escrita direta**, sem `confirmar_acao`, como `anexar_arquivo`. O conteúdo enviado SUBSTITUI tudo: sem `esperado_modificado_em`, quem chama tem de reler antes por conta própria.
