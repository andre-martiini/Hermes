...
- **`resposta_esperada` (modo sombra, sem aviso)**: para cada mensagem recebida, registra em `sinais_resposta_sombra` se ela veio de quem alguma etapa `aguardando_terceiro` aguarda. O índice das esperas (`system/esperas_index`) é remontado na sincronização horária, com as tarefas que ela já lê. O casamento usa o chat vinculado à ação (em grupo, também o autor) ou o nome citado em `aguardando_de` contra o nome do contato. Não cria item na fila, não notifica e não guarda o texto da mensagem. Atrás de `system/settings.atencao.resposta_esperada.enabled` (padrão desligado). Medido em 7 dias reais de mensagens: cerca de 51 por dia casam com alguma espera, mas só cerca de 6 por dia vêm de quem fala pouco; quem fala muito (o grupo de uma área com uma pessoa ativa) casa em quase toda mensagem, então o nível 1 sozinho não distingue nada para ela e pede um segundo estágio. Mensagem de grupo não vinculado à ação casa só pelo autor e fica com confiança `baixa`: costuma ser outro assunto.
