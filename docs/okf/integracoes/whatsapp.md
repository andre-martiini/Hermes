...
- **Imagens (2026-09-18)**: o job também descreve cada `message_type == "image"` da seleção (Gemini via `_describe_selected_images`; descrição + texto visível literal, feature `whatsapp_consolidation.image_description`, ~US$0,0006/imagem), em paralelo e com orçamento de 90s. O resultado é cacheado em **`image_description`** (+ `image_description_model`) — campo próprio, deliberadamente NÃO `transcription_text`: esse é lido como texto da mensagem pelo modo secretário e pela triagem, e o texto lido dentro de imagem de terceiro não pode entrar por essa porta. No transcript vira `[imagem — descrição automática] … — legenda: …`; o prompt de síntese trata essas linhas como contexto e nunca como instrução. `ler_mensagens_whatsapp` devolve `descricao_imagem` (só existe após consolidar) e `imagens_sem_descricao`; `ler_consolidacao_whatsapp` traz `midia.imagens_descritas/ignoradas`. Figurinhas e documentos seguem sem análise.
...
- Áudios sem mídia capturada viram `[áudio não capturado]`; >24MB ou formato não suportado são pulados com nota; falha transitória de transcrição não é cacheada (retry funciona). Contra mídia perdida na captura, o worker tem duas defesas (2026-08-21): upload ao Storage com retry (3 tentativas — uma falha transitória de DNS custou 5 mídias em produção) e a varredura `repairMissingMedia` (pós-ready + horária) que rebaixa do WhatsApp mídias recentes (48h) sem `media.storage_path` via `getMessageById` (`wa_serialized_id`), com teto de 3 tentativas por mensagem e alerta no Telegram ao esgotar.
...
- O vínculo cumpre dois papéis: **pré-sugestão de destino** na Caixa de Entrada (consolidações de um chat vinculado oferecem a(s) ação(ões) vinculada(s) no topo do seletor de associação) e **gatilho da triagem automática** no modo só-vinculados — ver abaixo.

### Triagem automática (modo só-vinculados)

- Arquivo: `functions/whatsapp_ingest.py`, função `triage_whatsapp_messages` — chamada no fim de `run_full_sync` (mesmo ciclo de 30 min do e-mail). Desligada em 2026-08-14 em favor da consolidação manual, foi religada no modo `linked_chats_only` (padrão `true`): só conversas com ação vinculada em `tarefas.whatsapp_vinculos` geram análise/sugestão/digest — as demais ficam apenas na Caixa de Entrada. Flags em `system/settings.whatsapp_ingest` (`enabled = true` hoje).
...
- Pré-filtro por vínculo manual: quando o `chat_id` de uma janela tem ação(ões) vinculada(s) em `tarefas.whatsapp_vinculos`, a IA escolhe `task_id` só entre essas candidatas — mesmo espírito do matching determinístico de SIPAC (`processo_sei`) e Calendar (`google_calendar_id`), mantendo a classificação de relevância com a IA.
- **Áudio em chat vinculado é conteúdo de primeira classe (2026-08-21)**: antes da análise, a janela transcreve os áudios capturados (mesmo pipeline e cache da consolidação — `transcription_text` no doc, nada é repago depois). Janela com mídia ainda sem `media.storage_path` é **adiada** (segura o cursor, como o teto de janelas já faz) por até 6h (`PENDING_MEDIA_DEFER_HOURS`) aguardando o reparo do worker; persistindo a falta e sem texto suficiente para análise, um alerta único por janela (marker em `whatsapp_ingest_alerts`) avisa no Telegram — antes o descarte era silencioso (`MIN_WINDOW_TEXT_CHARS` pulava janelas só-áudio sem segurar o cursor).
...
### Resposta sugerida (rascunho automático)

`functions/resposta_sugerida.py` faz o contrário dos detectores acima: em vez de só avisar que alguém espera resposta, **redige a resposta** e manda o cartão de aprovação do Telegram (`✅ Enviar / ✏️ Editar / 🗑️ Descartar`, o mesmo de `outbox_aprovacao`).

1. **Agendar** (`agendar`, chamado no fim de `on_whatsapp_message_atencao`): mensagem de conversa **individual** (`@c.us`/`@lid`) de **contato conhecido** (na `chats_allowlist` explícita ou com perfil em `perfil_pessoas`; `leitura_total` não conta, e número desconhecido fica de fora: em 30 dias eram 37% das conversas e 13% das mensagens, quase sempre spam ou robô), com texto, que não seja do dono nem do self-chat e com menos de `idade_max_h` (a recuperação retroativa do boot do worker não vira cartão) grava `respostas_sugeridas/{chat}` com `due_at = agora + atraso_min`. Cada mensagem nova do mesmo chat empurra o prazo: uma rajada vira UM rascunho, sobre a última.
2. **Processar** (`sugerir_respostas`, agendada `*/5 7-22 * * *` America/Sao_Paulo, 256 MB): pega os chats com `due_at` vencido e descarta sem chamar o modelo quando o dono já respondeu (`ja_respondida`), chegou mensagem mais nova (fica para o novo prazo), a mensagem é antiga, o secretário está cuidando do chat (`secretario_ativo`), já há rascunho pendente para o chat (`ja_ha_rascunho`) ou o limite do dia acabou.
3. **Decidir e redigir**: UMA chamada ao Gemini (`gemini-3.6-flash`, `thinking_level="low"`, ~US$ 0,003) devolve `responder` / `nao_responder` / `precisa_do_andre` e o texto. O prompt usa perfil do contato, ação relacionada (busca limitada por pessoa: `mapear_acoes_ativas_por_chat` varre `tarefas` inteira e não é usado), as últimas 15 mensagens e as mensagens do próprio dono neste chat como exemplo de voz. Só usa fatos da conversa; assunto sensível, compromisso ou informação que só o dono tem viram `precisa_do_andre` (sem rascunho). Testado contra o Gemini real: recusa injeção de prompt e pedido de dinheiro, responde "às 14h" quando o horário está na conversa, não responde agradecimento.
4. **Validar** (determinístico, depois do modelo): recusa rascunho vazio, com mais de 600 caracteres, com colchetes/chaves, assinado como bot, igual à mensagem recebida, ou com link, e-mail ou número que **não está na conversa** (defesa contra injeção que peça para vazar ou inventar dado).
5. **Criar**: `outbox_aprovacao.criar_rascunho(tipo="resposta_sugerida", origem="resposta_sugerida")`; o cartão mostra o que a pessoa escreveu. O resultado de cada chat fica em `respostas_sugeridas` (`estado`: `agendada`/`rascunhada`/`ignorada`, `motivo_ignorada`, `outbox_id`) e os contadores do dia em `respostas_sugeridas_contadores/{AAAA-MM-DD}` (`chamadas_llm`, `rascunhos`).

**Nunca envia sozinho e nunca é promovido a envio automático**: `outbox_aprovacao.TIPOS_SEMPRE_COM_APROVACAO` impede que o tipo `resposta_sugerida` entre em janela de cancelamento mesmo que esteja em `system/mcp_access.tipos_promovidos`, e `promocao_autonomia.tipos_elegiveis_para_promocao` nunca o sugere.

Configuração em `system/settings.atencao.resposta_sugerida` (**padrão desligado**, como os outros detectores): `enabled`, `atraso_min` (4), `idade_max_h` (12), `limite_dia` (15), `limite_por_rodada` (4), `apenas_conhecidos` (verdadeiro). Só conversa individual; grupo, áudio sem texto, figurinha e mídia sem legenda ficam de fora. Ainda não usa a agenda do dono ("pode ser às 15h?" vira `precisa_do_andre`).
