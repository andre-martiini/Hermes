...
- Janelas `acao` propõem o vínculo pelo mesmo motor de sugestão usado por e-mail/SIPAC/Calendar (`functions/email_action_linker.py:queue_and_maybe_send_suggestion`) — cartão de confirmação no Telegram, canal `whatsapp`.
...
- Janelas `acao`/`conhecimento` (não `ruido`) geram um **digest** em `whatsapp_digests/{chat_id}_{timestamp}`: `chat_id`, `chat_name`, `resumo`, `topicos[]`, `relevancia`, `n_mensagens`, `inicio`/`fim`, `embedding` (Vector 768-dim, `gemini-embedding-001`).
