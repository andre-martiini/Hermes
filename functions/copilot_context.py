"""Constroi o contexto textual do copiloto para consumo por clientes MCP
(resource hermes://voice-context). Adaptado de hermes-voice-bridge/context.py,
mas usa o uid autenticado (do ID token verificado) em vez de uma env var fixa,
e o client firebase_admin.firestore ja inicializado pelas Cloud Functions.
"""
...
def build_mcp_voice_context(db, uid: str | None = None) -> str:
    sections = [
        _format_core_context(db),
        _format_user_profile(db, uid),
...
def _format_core_context(db) -> str:
    ...
    soul = _get_doc_dict(db, "system", "copilot_soul")
...
def _format_user_profile(db, uid: str | None) -> str:
    user_doc = None
    ...
    if history:
        compact = [
            {"texto": item.get("texto"), "at": item.get("at")}
