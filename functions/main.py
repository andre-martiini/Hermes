...
def check_and_send_reminders(event: scheduler_fn.ScheduledEvent) -> None:
    ...
    # Definicao em health_routines.DEFAULT_HEALTH_REMINDERS (fonte unica no lado
    # Python, compartilhada com o Resumo Matinal). O que estiver gravado em
    # health_telegram_reminders sobrepoe estas entradas, por id.
    default_health_reminders = DEFAULT_HEALTH_REMINDERS
...
# ---------------------------------------------------------------------------
# REMOVIDO (17/09/2026): pipeline contexto_agente (eh_acao_critica,
# montar_texto_fonte_contexto, calcular_hash_contexto, construir_prompt_contexto,
# parse_resposta_contexto, processar_contexto_agente,
# on_tarefa_written_contexto_agente). Gatilho em toda criacao/edicao de tarefa
# critica que pre-computava um resumo estruturado via Gemini e gravava em
# tarefas/{id}.contexto_agente. Unico consumidor era obter_acao (tools/hermes_tools.py),
# ou seja, so existia para dar a MIM (Claude) um resumo pronto -- mas eu ja leio
# titulo/descricao/plano_acao/diario completos via obter_acao, entao consigo
# montar o mesmo resumo sozinho, sem custo de API. Nao renderizado em nenhuma
# tela do frontend (conferido em src/). O campo contexto_agente continua no
# schema de saida de obter_acao (dados antigos ainda aparecem); so parou de ser
# ESCRITO. Ver diario da acao b066fbd2-8552-4321-b.
# ---------------------------------------------------------------------------
