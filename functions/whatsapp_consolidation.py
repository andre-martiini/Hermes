...
que: transcreve os áudios (Groq/Whisper com fallback Gemini, cacheando o
resultado de volta em `whatsapp_messages.transcription_text`), monta o
transcript literal POR CÓDIGO (nunca pela IA — é a garantia anti-alucinação),
...
def _audio_ext_from_storage_path(storage_path: str) -> str:
...
def _build_transcript(messages: list[dict]) -> str:
    ...
    data muda, uma linha `[HH:mm] Autor: conteúdo` por mensagem. Áudios usam a
    transcrição (ou o placeholder de indisponibilidade) preenchida em
    `_transcribe_selected_audios`; imagens/mídia visual viram `[tipo]` + legenda."""
    lines: list[str] = []
...
- Trechos "[áudio sem transcrição]", "[áudio não capturado]" ou "[Transcrição indisponível...]" indicam conteúdo inacessível — não especule sobre eles.
"""
...
def _transcribe_selected_audios(db, job_ref, messages: list[dict], refs_by_id: dict) -> tuple[int, int]:
    ...
    for i, msg in enumerate(audio_msgs, start=1):
        ...
        if int(media.get("sizeBytes") or 0) > MAX_AUDIO_BYTES:
            ...
            n_ignorados += 1
            continue
        ext = _audio_ext_from_storage_path(storage_path)
...
def process_consolidation_job(db, job_ref, job: dict) -> None:
    ...
    # 1. Transcrição (com cache) — preenche transcription_text em cada áudio.
    ...
    attachments = [
        ...
        and str(m.get("message_type")) not in AUDIO_MESSAGE_TYPES
    ]
    job_ref.set({
        "status": "completed",
        "progress": None,
