...
export const NFSeGenerator = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 bg-on-surface/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-surface rounded-none border-2 border-primary-tactile shadow-none w-full max-w-5xl max-h-[95vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        ...
        <div className="bg-on-surface text-surface p-6 flex justify-between items-center border-b border-primary-tactile shrink-0">
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
                ...
                <span className="text-[10px] font-mono font-bold uppercase tracking-[0.2em] opacity-50">STATUS: SYSTEM_READY</span>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-0 flex-1 overflow-hidden flex">
          ...
          <div className="md:col-span-2 p-8 border-r border-border-grid overflow-y-auto space-y-8 bg-surface">
            <div>
              <h3 className="text-[10px] font-mono font-bold uppercase text-primary-tactile tracking-[0.3em] mb-6 flex items-center gap-2">
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-2">NET_AMOUNT_RECEIVABLE (BRL)</label>
                  <input 
                    type="text" 
                    value={valorLiquido}
                    onChange={(e) => setValorLiquido(e.target.value)}
                    placeholder="0000,00"
                    className="w-full bg-white border border-border-grid rounded-soft-touch px-4 py-3 text-sm font-mono font-bold text-on-surface outline-none focus:ring-1 focus:ring-primary-tactile"
                  />
                </div>

                <div className="grid grid-cols-3 gap-3">
                  <div className="col-span-1">
                    <label className="block text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-2">COMPETENCE</label>
                    <input 
                      type="text" 
                      value={formatDataCompetenciaDisplay(dataCompetencia)}
                      onChange={handleDataCompetenciaChange}
                      placeholder="DDMMYYYY"
                      className="w-full bg-white border border-border-grid rounded-soft-touch px-4 py-2 text-xs font-mono font-bold text-on-surface outline-none focus:ring-1 focus:ring-primary-tactile"
                    />
                  </div>
                  <div className="col-span-1">
                    <label className="block text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-2">REF_MONTH</label>
                    <select 
                      value={selecaoMes}
                      onChange={(e) => setSelecaoMes(Number(e.target.value))}
                      className="w-full bg-white border border-border-grid rounded-soft-touch px-3 py-2 text-xs font-mono font-bold text-on-surface outline-none focus:ring-1 focus:ring-primary-tactile appearance-none"
                    >
                      {monthNames.map((m, i) => (
                  ...
                  <div className="col-span-1">
                    <label className="block text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-2">REF_YEAR</label>
                    <input 
                      type="number" 
                      value={selecaoAno}
                ...
                <div>
                  <label className="block text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-2">TAKER_TAX_ID (CNPJ)</label>
                  <div className="flex gap-2">
                    <input 
                      ...
                      maxLength={18}
                      className="flex-1 bg-white border border-border-grid rounded-soft-touch px-4 py-3 text-sm font-mono font-bold text-on-surface outline-none focus:ring-1 focus:ring-primary-tactile"
                    />
                    <button 
                      onClick={buscarCnpj}
                      disabled={loading || cleanCnpj(cnpj).length !== 14}
                      className="bg-primary-tactile text-white px-4 rounded-soft-touch font-mono font-bold text-[10px] uppercase tracking-widest disabled:opacity-50 hover:bg-on-surface transition-all flex items-center justify-center min-w-[50px]"
                    >
                      {loading ? (
            ...
            <div className="bg-slate-50 rounded-none p-6 border border-border-grid space-y-6">
                <h4 className="text-[8px] font-mono font-bold text-primary-tactile uppercase tracking-widest">// TAX_OVERRIDE_INTERFACE</h4>
                
                <div className="grid grid-cols-3 gap-3">
                    <div>
                        <label className="block text-[7px] font-mono font-bold text-slate-400 uppercase mb-1">BRUTO_OVR</label>
                        <input 
                            type="text" 
                            placeholder={brutoAutoNum.toFixed(2).replace('.', ',')}
                            value={valorBrutoManual || ''}
                            onChange={(e) => setValorBrutoManual(e.target.value)}
                            className="w-full bg-white border border-border-grid rounded-none px-3 py-2 text-[10px] font-mono font-bold text-on-surface outline-none focus:ring-1 focus:ring-primary-tactile"
                        />
                    </div>
                    <div>
                        <label className="block text-[7px] font-mono font-bold text-slate-400 uppercase mb-1">CP_INSS_OVR</label>
                        <input 
                            type="text" 
                    ...
                    <div>
                        <label className="block text-[7px] font-mono font-bold text-slate-400 uppercase mb-1">IRRF_OVR</label>
                        <input 
                            type="text" 
                            placeholder="0,00"
                ...
                <div>
                    <label className="block text-[7px] font-mono font-bold text-slate-400 uppercase mb-1">MANUAL_DESCRIPTION_INJECTION</label>
                    <textarea 
                        value={finalDescricao}
                        onChange={(e) => setDescricaoManual(e.target.value)}
                        className="w-full bg-white border border-border-grid rounded-none px-3 py-2 text-[10px] font-mono font-bold text-on-surface outline-none focus:ring-1 focus:ring-primary-tactile min-h-[80px] resize-none"
                    />
                </div>
            </div>

            {/* Acesso ao Portal (Login Automático) */}
            <div className="bg-on-surface rounded-none p-6 text-white space-y-6">
                <div className="flex items-center gap-2 mb-1">
                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div>
                    <h4 className="text-[8px] font-mono font-bold text-emerald-400 uppercase tracking-widest">// AUTO_LOGIN_ACCESS_RESOURCES</h4>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-[7px] font-mono font-bold text-white/40 uppercase mb-1">ACCESS_ID</label>
                        <input 
                            type="text" 
                            value={portalLogin}
                            onChange={(e) => setPortalLogin(e.target.value)}
                            className="w-full bg-white/5 border border-white/10 rounded-none px-3 py-2 text-[10px] font-mono font-bold text-white outline-none focus:ring-1 focus:ring-emerald-500"
                            placeholder="USER_IDENTIFIER"
                        />
                    </div>
                    <div>
                        <label className="block text-[7px] font-mono font-bold text-white/40 uppercase mb-1">SECURE_TOKEN</label>
                        <input 
                            type="password" 
                            value={portalSenha}
                ...
                <p className="text-[7px] font-mono font-bold text-white/30 leading-relaxed uppercase tracking-widest">
          ...
          <div className="md:col-span-3 bg-slate-50 p-8 overflow-y-auto relative bg-[url('https://www.transparenttextures.com/patterns/graphy.png')]">
            {!showResults ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-300 p-12 text-center">
                ...
                <h4 className="font-mono font-bold text-[10px] uppercase tracking-[0.2em]">WAITING_FOR_DATA_STREAM</h4>
                <p className="font-mono text-[8px] uppercase tracking-widest mt-2">INITIALIZE_PROCEDURE_BY_FILLING_INPUT_PARAMETERS</p>
              </div>
            ) : (
              <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                
                {/* Copiar Block Helper */}
                <div className="flex items-center gap-3 bg-white p-4 border border-border-grid border-l-4 border-l-emerald-500">
                    <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                    <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-on-surface">GENERATED_MIRROR_STREAM // READY_FOR_TRANSFER</span>
                </div>

                <div className="space-y-6">
                    {/* Linha 1: Códigos */}
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white p-6 rounded-none border border-border-grid shadow-none">
                            <span className="block text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-2">TAKER_ID (CNPJ)</span>
                            <div className="flex justify-between items-center">
                                <span className="text-sm font-mono font-bold text-on-surface">{clientData?.cnpj || cnpj}</span>
                                <button 
                        ...
                        <div className="bg-white p-6 rounded-none border border-border-grid shadow-none">
                            <span className="block text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-2">TAX_CLASSIFICATION_CODE</span>
                            <div className="flex justify-between items-center">
                                <span className="text-sm font-mono font-bold text-on-surface">17.01</span>
                                <button 
                    ...
                    <div className="bg-white p-6 rounded-none border border-border-grid shadow-none">
                        <div className="flex justify-between items-start mb-4">
                            <span className="block text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest">SERVICE_DESCRIPTION_BLOCK</span>
                            <button 
                                onClick={() => handleCopy(finalDescricao, 'descricao')}
                                className={`flex items-center gap-2 px-4 py-2 rounded-soft-touch text-[9px] font-mono font-bold uppercase tracking-widest transition-all ${copiedField === 'descricao' ? 'bg-emerald-500 text-white' : 'bg-on-surface text-white hover:bg-primary-tactile'}`}
                            >
                        ...
                        <p className="text-[11px] font-mono font-bold text-on-surface leading-relaxed bg-slate-50 p-4 border border-border-grid border-dashed">
                            {finalDescricao}
                        </p>
                    </div>
                    ...
                    {/* Valores Críticos */}
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-surface p-6 rounded-none border border-border-grid border-l-4 border-l-primary-tactile relative overflow-hidden group">
                            <span className="block text-[8px] font-mono font-bold text-primary-tactile uppercase tracking-widest mb-2">GROSS_SERVICE_AMOUNT</span>
                            <div className="text-2xl font-mono font-bold text-on-surface">{formatCurrency(finalBruto)}</div>
                            <button 
                        ...
                        <div className="bg-surface p-6 rounded-none border border-border-grid border-l-4 border-l-accent-tactile relative overflow-hidden group">
                            <span className="block text-[8px] font-mono font-bold text-accent-tactile uppercase tracking-widest mb-2">INSS_CP_RETAINED (11%)</span>
                            <div className="text-2xl font-mono font-bold text-on-surface">{formatCurrency(finalCP)}</div>
                            <button 
                    ...
                    <div className="pt-2 flex flex-col gap-4">
                        {robotStatus === 'processing' && isRobotRunning && (
                          <button
                            ...
                            className="w-full py-4 bg-emerald-50 text-emerald-700 rounded-none border border-emerald-200 font-mono font-bold text-[10px] uppercase tracking-[0.2em] hover:bg-emerald-100 transition-all flex items-center justify-center gap-3 animate-pulse"
                          >
                        ...
                        <button
                            ...
                            className={`w-full py-5 rounded-none font-mono font-bold uppercase tracking-[0.3em] text-xs flex items-center justify-center gap-4 transition-all shadow-none border-2 ${
                    ...
                    <div className="bg-white p-6 rounded-none border border-border-grid border-dashed">
                        <span className="block text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-4 text-center">// SECONDARY_TAX_STREAMS (NULL_VALS)</span>
                        <div className="grid grid-cols-5 gap-3">
                           {['ISS', 'PIS', 'COFINS', 'CSLL', 'IR'].map(imposto => (
                               <div key={imposto} className="text-center group border-r border-border-grid last:border-0">
                                   <div className="text-[8px] font-mono font-bold text-slate-500 uppercase">{imposto}</div>
                                   <div className="text-[10px] font-mono font-bold text-slate-300 mt-2 cursor-pointer hover:text-primary-tactile transition-colors" onClick={() => handleCopy('0,00', `imposto_${imposto}`)}>0,00</div>
                               </div>
                           ))}
                        </div>
                    </div>
