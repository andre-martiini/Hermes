...
interface QuickLogModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddLog: (text: string, systemId: string) => void;
  unidades: { id: string; nome: string }[];
}

export const QuickLogModal = ({ isOpen, onClose, onAddLog, unidades }: QuickLogModalProps) => {
      ...
      reader.onloadend = async () => {
        try {
          ...
          const response = await transcribeFunc({
            ...
            sourceRef: { kind: 'unknown', label: 'quick_log_modal' },
          });
