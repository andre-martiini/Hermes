...
export const QuickNoteModal: React.FC<QuickNoteModalProps> = ({ isOpen, onClose, onAddIdea, showAlert }) => {
  ...
  return (
    <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-slate-950/60 animate-in fade-in">
      <div className="bg-white w-full max-w-2xl rounded-lg md:rounded-[2.5rem] shadow-lg overflow-hidden animate-in zoom-in-95">
        <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
          <div>
            <h3 className="text-2xl font-black text-slate-900 tracking-tight">Nota Rápida</h3>
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider mt-1">Captação Instantânea</p>
          </div>
        ...
        <div className="p-8 space-y-6">
          <div className="bg-slate-50 p-2 rounded-lg md:rounded-2xl border-2 border-slate-100 flex items-center gap-4 focus-within:border-blue-500 transition-all">
            <button
              onClick={isRecording ? stopRecording : startRecording}
              disabled={isProcessing}
              className={`p-4 rounded-lg md:rounded-xl transition-all flex-shrink-0 ${isRecording
            ...
            <button
              ...
              className="bg-blue-600 text-white p-4 rounded-lg md:rounded-xl hover:bg-blue-700 transition-all shadow-xl shadow-blue-100"
            >
