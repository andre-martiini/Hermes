          ...
          <div className="space-y-1 pb-2">
                      ...
                      <div className={`p-4 rounded-lg border max-w-full md:max-w-[90%] shadow-lg relative group ${isDark ? 'bg-[#1A1A1A] border-[#e5e7eb] dark:border-white/10' : 'bg-white border-[#e5e7eb] dark:border-white/10 shadow-slate-200'}`}>
                        ...
                        <div className="flex items-center justify-between mt-2 gap-4">
                          ...
                          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => handleCopyMessage(entry.nota)} className={`transition-colors ${isDark ? 'text-white/40 hover:text-emerald-400' : 'text-slate-400 hover:text-emerald-500'}`} title="Copiar">
                            ...
                            <button onClick={() => handleEditDiaryEntry(idx)} className={`transition-colors ${isDark ? 'text-white/40 hover:text-blue-400' : 'text-slate-400 hover:text-blue-500'}`} title="Editar">
                              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                            </button>
                            <button onClick={() => handleDeleteDiaryEntry(idx)} className={`transition-colors ${isDark ? 'text-white/40 hover:text-rose-500' : 'text-slate-400 hover:text-rose-500'}`} title="Excluir">
            ...
            {(!currentTaskData.acompanhamento || currentTaskData.acompanhamento.length === 0) && (
              <div className="flex-1 flex flex-col items-center justify-center text-center opacity-30 mt-16">
                <p className={`text-sm font-medium mb-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>Tudo pronto para começar?</p>
                <p className={`text-xs ${isDark ? 'text-white/60' : 'text-slate-500'}`}>Registre seu diário de execução abaixo.</p>
              </div>
            )}
          ...
          <div className="relative shrink-0">
            ...
            {showAttachMenu && (
              <div className={`absolute bottom-11 left-0 w-48 rounded-lg border shadow-xl overflow-hidden animate-in zoom-in-95 origin-bottom-left z-[100] ${isDark ? 'bg-[#1A1A1A] border-[#e5e7eb] dark:border-white/10' : 'bg-white border-[#e5e7eb] dark:border-white/10'}`}>
                ...
                <button onClick={() => { setModalConfig({ type: 'link', isOpen: true }); setShowAttachMenu(false); }} className={`w-full text-left px-4 py-3 text-xs font-bold flex items-center gap-2 ${isDark ? 'text-white/80 hover:bg-white/10' : 'text-slate-700 hover:bg-slate-50'}`}>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
                  Inserir Link
                </button>
                <button onClick={() => { setModalConfig({ type: 'contact', isOpen: true }); setShowAttachMenu(false); }} className={`w-full text-left px-4 py-3 text-xs font-bold flex items-center gap-2 ${isDark ? 'text-white/80 hover:bg-white/10' : 'text-slate-700 hover:bg-slate-50'}`}>
