...
export const PainelControleUI = ({
  ...
  return (
    <div className="flex flex-col gap-4 md:gap-6 overflow-hidden px-0">
      ...
      <div className="bg-gradient-to-br from-indigo-600 to-blue-700 rounded-lg md:rounded-[1.5rem] p-3 md:p-4 text-white shadow-lg flex-shrink-0 relative overflow-hidden group">
        ...
        <div className="relative z-10 flex items-center gap-3">
          <div className="relative flex-1">
            <input
              ...
              className="w-full bg-black/20 border border-white/10 rounded-lg md:rounded-xl px-3 py-1.5 text-[10px] md:text-xs font-medium focus:ring-2 focus:ring-white/30 outline-none text-white placeholder:text-white/20 transition-all"
            />
            {chatUrl !== (task.chat_gemini_url || '') && (
              <button
                onClick={handleSaveChatUrl}
                className="absolute right-1 top-1 bottom-1 bg-emerald-500 text-white px-2 rounded-lg md:rounded-lg text-[7px] font-black uppercase"
              >
                OK
              </button>
            )}
          ...
          <a
            ...
            className="bg-white text-indigo-600 w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-lg md:rounded-xl shadow-xl flex-shrink-0"
          >
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className={`h-16 flex flex-col items-center justify-center gap-1 rounded-lg md:rounded-2xl transition-all border ${nextPendingReminder ? 'bg-amber-500 text-white border-amber-600 shadow-lg' : 'bg-white text-slate-400 border-slate-200 hover:bg-slate-50'}`}
        >
          ...
          <span className="text-[8px] font-bold uppercase tracking-wider">Lembrete</span>
        </button>

        <button
          onClick={handleCompleteTaskRequest}
          className="h-16 flex flex-col items-center justify-center gap-1 rounded-lg md:rounded-2xl bg-emerald-50 text-emerald-600 border border-emerald-100 hover:bg-emerald-100 transition-all shadow-sm"
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" strokeWidth="3" /></svg>
          <span className="text-[8px] font-bold uppercase tracking-wider">Concluir</span>
        </button>
      </div>
    </div>
  );
