...
export const ServicesView: React.FC<ServicesViewProps> = ({
  ...
  return (
    <div className="space-y-12 animate-in fade-in pb-20 bg-surface min-h-screen p-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b-2 border-primary-tactile pb-6">
            <div>
                ...
                <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest">STATUS: CONNECTED</span>
                </div>
            </div>
            <div className="flex gap-1 bg-slate-100 p-1 rounded-none border border-border-grid">
                <button
                    onClick={() => setActiveTab('dashboard')}
                    className={`px-6 py-3 text-[10px] font-mono font-bold uppercase tracking-[0.2em] transition-all ${activeTab === 'dashboard' ? 'bg-white text-on-surface border border-border-grid shadow-sm' : 'text-slate-400 hover:text-on-surface hover:bg-white/50'}`}
                >
                    DASHBOARD_OVERVIEW
                </button>
                <button
                    onClick={() => setActiveTab('list')}
                    className={`px-6 py-3 text-[10px] font-mono font-bold uppercase tracking-[0.2em] transition-all ${activeTab === 'list' ? 'bg-white text-on-surface border border-border-grid shadow-sm' : 'text-slate-400 hover:text-on-surface hover:bg-white/50'}`}
                >
                    SERVICE_INVENTORY
                </button>
            </div>
        ...
        {/* --- Dashboard Tab --- */}
        {activeTab === 'dashboard' && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-border-grid bg-white">
                {/* MRR Card */}
                <div className="p-8 border-r border-border-grid flex flex-col justify-between relative overflow-hidden group">
                    <div className="relative z-10">
                        <h4 className="text-[10px] font-mono font-bold text-primary-tactile uppercase tracking-[0.25em] mb-4 flex items-center gap-2">
                        ...
                        <p className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-widest mt-4 opacity-70">ANNUAL_RECURRING_METRIC_ESTIMATE</p>
                    </div>
                </div>
                ...
                <div className="p-8 border-r border-border-grid flex flex-col justify-between relative overflow-hidden group">
                    <div className="relative z-10">
                        <h4 className="text-[10px] font-mono font-bold text-emerald-600 uppercase tracking-[0.25em] mb-4 flex items-center gap-2">
                        ...
                        <p className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-widest mt-4 opacity-70">PENDING_INSTALLMENTS_AGGREGATE</p>
                    </div>
                </div>
                ...
                <div className="p-8 flex flex-col justify-between relative overflow-hidden group">
                    <div className="relative z-10">
                        <h4 className="text-[10px] font-mono font-bold text-amber-600 uppercase tracking-[0.25em] mb-4 flex items-center gap-2">
                        ...
                        <p className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-widest mt-4 opacity-70">ACTIVE_CONTRACT_LOAD_SIGNAL</p>
                    </div>
                </div>
            </div>
        )}
            ...
            <div className="bg-surface rounded-none border border-border-grid shadow-none overflow-hidden min-h-[500px]">
                <div className="flex items-center justify-between p-8 border-b border-border-grid bg-slate-50/50">
                    ...
                    <button
                        ...
                        className="bg-primary-tactile text-white px-6 py-3 rounded-soft-touch text-[10px] font-mono font-bold uppercase tracking-widest shadow-sm hover:bg-on-surface transition-all flex items-center gap-3"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 border-t border-border-grid">
                    {services.map(service => (
                        <div
                            key={service.id}
                            onClick={() => setSelectedServiceId(service.id)}
                            className="group bg-white border border-border-grid p-8 hover:bg-slate-50 transition-all relative overflow-hidden cursor-pointer flex flex-col justify-between"
                        >
                            <div>
                                <div className="flex items-start justify-between mb-6">
                                    <span className={`text-[8px] font-mono font-bold px-2 py-0.5 border border-border-grid uppercase tracking-widest ${
                                    ...
                                    <span className="text-[8px] font-mono font-bold text-slate-400 uppercase tracking-[0.2em]">
                            ...
                            <div className="pt-6 border-t border-border-grid border-dashed flex items-center justify-between">
                                <div className="text-xl font-mono font-bold text-on-surface tracking-tighter">
                                    R$ {service.valor_total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                                </div>
                                <div className="text-primary-tactile opacity-0 group-hover:opacity-100 transition-all font-mono font-bold text-[9px] uppercase tracking-widest flex items-center gap-2">
                                    ACCESS_DETAILS
                    ...
                    {services.length === 0 && (
                        <div className="col-span-full py-24 text-center bg-slate-50/50">
                            <p className="text-slate-400 font-mono font-bold uppercase tracking-[0.3em] text-[10px]">NULL_DATASET // ZERO_SERVICES_REGISTERED</p>
                        </div>
                    )}
                </div>
            </div>
        ...
        {selectedService && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-on-surface/80 backdrop-blur-sm animate-in fade-in duration-300">
                <div className="bg-surface w-full max-w-5xl rounded-none border-2 border-primary-tactile shadow-none flex flex-col max-h-[95vh] overflow-hidden animate-in zoom-in-95">

                    {/* Header Industrial */}
                    <div className="p-8 border-b border-border-grid bg-slate-50 flex items-center justify-between flex-shrink-0">
                        ...
                        <div className="flex items-center gap-3">
                            {selectedService.status === 'Prospecção' && (
                                <button 
                                    ...
                                    className="bg-emerald-600 text-white px-5 py-3 rounded-soft-touch text-[10px] font-mono font-bold uppercase tracking-widest shadow-sm hover:bg-emerald-700 transition-all flex items-center gap-2"
                                >
                                    ACTIVATE_CONTRACT
                                </button>
                            )}
                            {selectedService.status === 'Ativo' && (
                                <button 
                                    ...
                                    className="bg-white border border-rose-200 text-rose-600 px-5 py-3 rounded-soft-touch text-[10px] font-mono font-bold uppercase tracking-widest hover:bg-rose-50 transition-all flex items-center gap-2"
                                >
                                    TERMINATE_SERVICE
                                </button>
                            )}
                            ...
                            <button onClick={() => setSelectedServiceId(null)} className="p-3 text-slate-400 hover:bg-slate-100 rounded-soft-touch transition-all border border-border-grid">
                    ...
                    <div className="p-10 flex-1 overflow-y-auto space-y-12 bg-white">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            <div className="space-y-6">
                                <div>
                                    <label className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-widest block mb-2">RESOURCE_IDENTIFIER (CLIENT)</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-widest block mb-2">OPERATIONAL_ROLE_SPEC</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-widest block mb-2">PROJECT_SCOPE_LOG</label>
                                    <textarea
                                        ...
                                        className="w-full bg-white border border-border-grid rounded-soft-touch px-4 py-3 text-xs font-mono font-bold text-on-surface outline-none focus:ring-1 focus:ring-primary-tactile min-h-[120px] resize-none"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-widest block mb-2">INITIAL_DATE</label>
                                        <input
                                            type="date"
                                    ...
                                    <div>
                                        <label className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-widest block mb-2">TERMINATION_ETA</label>
                                        <input
                                            type="date"
                                            disabled={localDoc?.status === 'Prospecção'}
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-widest block mb-2">HOURLY_LOAD (H/W)</label>
                                        <input
                                            type="number"
                                            disabled={localDoc?.status === 'Prospecção'}
                                ...
                                <div>
                                    <label className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-widest block mb-2">TOTAL_VAL_AGREEMENT (BRL)</label>
                                    <div className="relative">
                                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-mono font-bold text-primary-tactile">R$</span>
                                        <input
                                            type="number"
                                            ...
                                            className="w-full bg-slate-50 border border-primary-tactile rounded-soft-touch pl-10 pr-4 py-4 text-2xl font-mono font-bold text-on-surface outline-none focus:ring-1 focus:ring-primary-tactile"
                                        />
                                    </div>
                                    <button 
                                        onClick={handleUpdateTotalFromParcelas}
                                        disabled={!localDoc?.parcelas || localDoc.parcelas.length === 0}
                                        className="mt-3 text-[9px] font-mono font-bold text-primary-tactile hover:underline disabled:text-slate-300 disabled:no-underline uppercase tracking-widest flex items-center gap-2 transition-all"
                                    >
                        ...
                        {/* Parcelas Section */}
                        <div className="pt-10 border-t border-border-grid border-dashed">
                            <div className="flex items-center justify-between mb-8">
                                <div>
                                    <h4 className="text-[10px] font-mono font-bold text-on-surface uppercase tracking-[0.3em] flex items-center gap-2">
                                        <span className="w-2 h-2 bg-primary-tactile"></span>
                                        FINANCIAL_SCHEDULE_STREAM
                                    </h4>
                                    <p className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-widest mt-1">PAYMENT_MILESTONES_AND_INSTALLMENTS</p>
                                </div>
                                <div className="flex items-center gap-3">
                                    <button
                                        onClick={() => setShowBatchForm(!showBatchForm)}
                                        className="bg-white border border-border-grid text-on-surface px-4 py-2 rounded-soft-touch text-[10px] font-mono font-bold uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center gap-2 shadow-sm"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="bg-on-surface text-white px-4 py-2 rounded-soft-touch text-[10px] font-mono font-bold uppercase tracking-widest hover:bg-primary-tactile transition-all flex items-center gap-2 shadow-md"
                                    >
                                        + APPEND_INSTALLMENT
                                    </button>
                                </div>
                            ...
                            {showBatchForm && (
                                <div className="bg-slate-50 border border-border-grid p-8 rounded-none mb-10 animate-in slide-in-from-top-4 duration-300">
                                    <h5 className="text-[9px] font-mono font-bold text-primary-tactile uppercase tracking-widest mb-6 border-b border-border-grid pb-2">// BATCH_GENERATOR_CONFIG</h5>
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                                        <div>
                                            <label className="text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-2 block">START_PERIOD</label>
                                            <input 
                                                type="date" 
                                                value={batchForm.start}
                                                onChange={e => setBatchForm(prev => ({ ...prev, start: e.target.value }))}
                                                className="w-full bg-white border border-border-grid rounded-none px-3 py-2 text-xs font-mono font-bold text-on-surface outline-none focus:ring-1 focus:ring-primary-tactile" 
                                            />
                                        </div>
                                        <div>
                                            <label className="text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-2 block">END_PERIOD</label>
                                            <input 
                                                type="date" 
                                                value={batchForm.end}
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                <label className="text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-2 block">DUE_DAY</label>
                                                <input 
                                                    type="number" 
                                                    min="1" max="31"
                                                    value={batchForm.day}
                                            ...
                                            <div>
                                                <label className="text-[8px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-2 block">VAL_BRL</label>
                                                <input 
                                                    type="number" 
                                                    placeholder="0,00"
                                        ...
                                        <button 
                                            ...
                                            className="w-full bg-primary-tactile text-white px-4 py-3 rounded-soft-touch text-[10px] font-mono font-bold uppercase tracking-widest hover:bg-on-surface transition-all shadow-sm"
                                        >
                                            GENERATE_BATCH_STREAM
                                        </button>
                                    </div>
                                    <p className="text-[8px] font-mono font-bold text-slate-400 mt-6 uppercase tracking-widest opacity-60">* INJECTING_PARCELAS_INTO_EXISTING_DATASET.</p>
                                </div>
                            )}

                            <div className="space-y-2">
                                {(localDoc?.parcelas || []).map((parcela, index) => (
                                    <div key={parcela.id} className="flex items-center gap-4 bg-white p-3 rounded-none border border-border-grid group hover:bg-slate-50 transition-all">
                                        <div className="w-8 h-8 rounded-none border border-border-grid bg-slate-50 flex items-center justify-center text-[10px] font-mono font-bold text-slate-400 shrink-0">
                                        ...
                                        <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4">
                                            <input
                                                ...
                                                className="bg-transparent border-none focus:ring-1 focus:ring-primary-tactile rounded-none px-3 py-2 text-xs font-mono font-bold text-on-surface w-full"
                                                placeholder="INSTALLMENT_LABEL"
                                            />
                                            <div className="flex items-center gap-2 bg-slate-50 border border-border-grid rounded-none px-3 py-2 w-full">
                                                <span className="text-[10px] font-mono font-bold text-primary-tactile">BRL</span>
                                                <input
                                                    type="number"
                                                    value={parcela.valor}
                                                    onChange={(e) => {
                                                    ...
                                                    className="border-none outline-none text-xs font-mono font-bold text-on-surface w-full bg-transparent"
                                                />
                                            </div>
                                            <input
                                                type="date"
                                                ...
                                                className="bg-transparent border-none focus:ring-1 focus:ring-primary-tactile rounded-none px-3 py-2 text-xs font-mono font-bold text-on-surface w-full"
                                            />
                                            <div className="px-3 py-2 text-[10px] font-mono font-bold w-full flex items-center h-full uppercase tracking-widest">
                                ...
                                {(localDoc?.parcelas || []).length === 0 && (
                                    <div className="py-12 text-center border border-border-grid border-dashed bg-slate-50/50">
                                        <p className="text-slate-400 font-mono font-bold uppercase tracking-[0.2em] text-[10px]">NULL_TIMELINE // NO_INSTALLMENTS_DETECTED</p>
                                    </div>
                                )}
                            </div>
                        </div>
