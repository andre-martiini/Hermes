                ...
                <div className={`shrink-0 px-4 py-3 flex items-center justify-between border-b ${isDark ? 'border-white/10' : 'border-slate-100'}`}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`flex items-center justify-center w-8 h-8 rounded-xl transition-all relative ${isAnalyzingInsight
                    ...
                    >
                      ...
                      {isAnalyzingInsight && (
                        <div className="absolute inset-0 rounded-xl border-2 border-indigo-400/30 border-t-transparent animate-spin" />
                      )}
                    </button>
          ...
          <div className={`relative w-full max-w-md rounded-2xl border shadow-2xl p-6 ${isDark ? 'bg-[#0f1724] border-white/10' : 'bg-white border-slate-200'}`}>
            ...
            {insightState.alvo === 'acoes' && insightState.acoesPropostas && insightState.acoesPropostas.length > 0 && (
              <div className="space-y-3 mb-5">
                ...
                {insightState.acoesPropostas.map((idea, idx) => (
                  <div key={idx} className={`rounded-xl border p-3 flex items-start justify-between gap-3 ${isDark ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {idea.tags?.map(tag => (
                          <span key={tag} className={`text-[8px] px-1.5 py-0.5 rounded-md font-bold uppercase ${isDark ? 'bg-emerald-500/10 text-emerald-400' : 'bg-emerald-50 text-emerald-600'}`}>
                            #{tag}
                          </span>
                        ))}
                      </div>
            ...
            <div className="flex gap-3">
              ...
              {insightState.alvo === 'acoes' && (
                <button
                  onClick={handleInsightDiscard}
                  className="flex-1 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-[11px] font-black uppercase tracking-widest transition-all shadow-lg"
                >
                  Entendido
                </button>
              )}
