...
shell.Run "cmd /c npm run dev > """ & logsDir & "\frontend.log"" 2>&1", HIDDEN, WAIT_ON_RETURN

WScript.Sleep 2000

shell.CurrentDirectory = baseDir & "\automations"
shell.Run "cmd /c python server.py > """ & logsDir & "\automations.log"" 2>&1", HIDDEN, WAIT_ON_RETURN

shell.CurrentDirectory = baseDir
shell.Run "cmd /c python hermes_cli.py watch > """ & logsDir & "\sync.log"" 2>&1", HIDDEN, WAIT_ON_RETURN
shell.Run "cmd /c python hermes_cli.py watch-pages > """ & logsDir & "\watch_pages.log"" 2>&1", HIDDEN, WAIT_ON_RETURN
